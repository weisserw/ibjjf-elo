...
class LivestreamFrameTextOcrFixtureTestCase(unittest.TestCase):
    ...
    def test_paddle_existing_name_fixtures_find_two_names(self):
        ...
        cases = [
            ...
            "raw_score_300_200.jpg",
            "raw_score_300_000.jpg",
        ]
    ...
    def test_paddle_name_fixture_names(self):
        ...
        cases = [
            ...
            (
                "raw_score_000_000.jpg",
                "MATHEUS DOS SANTOS DU",
                "NOAH RODRIGUES ALVES",
            ),
            (
                "raw_score_300_200.jpg",
                "MICHAEL S BRITORODRIQUEZ",
                "JAMES MITCHELL BRUNER",
            ),
            (
                "raw_score_victory.jpg",
                "Victory",
                "Isaac Jacob Roberts",
            ),
            (
    ...
    def test_standard_score_success_does_not_use_raw_fallback(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        score_path = os.path.join(self.fixture_dir, "score_000_000.jpg")
        ...
        with mock.patch.object(
            reader,
            "_read_raw_scoreboard",
            wraps=reader._read_raw_scoreboard,
        ) as raw_read:
            reading = reader.read(image)
    ...
    def test_raw_score_fixture_scores(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        parser = text_ocr.FrameImageTextParser("auto", "fixed_digit", "none")
        cases = [
            (
    ...
    def test_raw_score_detection_is_not_tied_to_original_crop_size(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        parser = text_ocr.FrameImageTextParser("auto", "fixed_digit", "none")
        score_path = os.path.join(self.fixture_dir, "raw_score_012_212.jpg")
        ...
        reading = parser.parse(0, buffer.getvalue(), None)

        self.assertEqual(reading.scoreboard_state, text_scan.SCOREBOARD_STATE_VISIBLE)
        self.assertEqual(reading.top_points, 0)
        self.assertEqual(reading.top_advantages, 1)
        self.assertEqual(reading.top_penalties, 2)
        self.assertEqual(reading.bottom_points, 2)
        self.assertEqual(reading.bottom_advantages, 1)
        self.assertEqual(reading.bottom_penalties, 2)

    def test_raw_score_popup_does_not_emit_score(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        parser = text_ocr.FrameImageTextParser("auto", "fixed_digit", "none")

        score_path = os.path.join(self.fixture_dir, "raw_score_error_ignore.jpg")
        with open(score_path, "rb") as fileobj:
            reading = parser.parse(0, fileobj.read(), None)

        self.assertIsNone(reading.scoreboard_state)
        for field_name in text_scan.SCORE_FIELDS:
            self.assertIsNone(getattr(reading, field_name))

    def test_raw_score_victory_marks_scoreboard_blank(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        parser = text_ocr.FrameImageTextParser("auto", "fixed_digit", "none")
        score_path = os.path.join(self.fixture_dir, "raw_score_victory.jpg")
    ...
    def test_raw_timer_fixture_values(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        parser = text_ocr.FrameImageTextParser("auto", "fixed_digit", "none")
        cases = [
            ("raw_timer_500_stopped.jpg", "stopped", "5:00"),
            ("raw_timer_000_stopped_dark.jpg", "stopped", "0:00"),
            ("raw_timer_037.jpg", "running", "0:37"),
            ("raw_timer_219.jpg", "running", "2:19"),
            ("raw_timer_256_stopped_dark.jpg", "stopped", "2:56"),
            ("raw_timer_438.jpg", "running", "4:38"),
            ("raw_timer_539_stopped.jpg", "stopped", "5:39"),
            ("raw_timer_556.jpg", "running", "5:56"),
        ]

        for timer_image, timer_state, timer_value in cases:
            with self.subTest(timer_image=timer_image):
                timer_path = os.path.join(self.fixture_dir, timer_image)
                self.assertTrue(
                    os.path.exists(timer_path),
                    f"missing livestream OCR timer fixture: {timer_image}",
                )
                with open(timer_path, "rb") as fileobj:
                    reading = parser.parse(0, None, fileobj.read())

                self.assertEqual(reading.timer_state, timer_state)
                self.assertEqual(reading.timer_value, timer_value)

    def test_raw_timer_detection_is_not_tied_to_original_crop_size(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        parser = text_ocr.FrameImageTextParser("auto", "fixed_digit", "none")
        timer_path = os.path.join(self.fixture_dir, "raw_timer_219.jpg")
        ...
        buffer = io.BytesIO()
        resized.save(buffer, format="JPEG", quality=95)
        reading = parser.parse(0, None, buffer.getvalue())

        self.assertEqual(reading.timer_state, "running")
        self.assertEqual(reading.timer_value, "2:19")

    def test_standard_timer_success_does_not_use_raw_fallback(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        timer_path = os.path.join(self.fixture_dir, "timer_stopped_000.jpg")
        reader = text_ocr.TimerDigitReader()
    ...
    def test_raw_scoreboard_crop_is_not_timer(self):
        text_ocr.validate_ocr_engines("fixed_digit", "none")
        parser = text_ocr.FrameImageTextParser("auto", "fixed_digit", "none")
        timer_path = os.path.join(self.fixture_dir, "raw_score_300_000.jpg")

        with open(timer_path, "rb") as fileobj:
            reading = parser.parse(0, None, fileobj.read())

        self.assertEqual(reading.timer_state, "blank")
        self.assertIsNone(reading.timer_value)
