...
def athletes():
    ...
    query = db.session.query(Athlete.name).filter(Athlete.name.ilike(f'{search}%')).order_by(Athlete.name).limit(MAX_RESULTS)
    results = query.all()
    ...
    if len(results) < MAX_RESULTS:
        ...
        additional_query = db.session.query(Athlete.name).filter(Athlete.name.ilike(f'%{search}%')).order_by(Athlete.name).limit(remaining_count)
