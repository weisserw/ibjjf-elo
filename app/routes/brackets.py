...
def archive_competitors():
    ...
    # If any matches have match_number set, filter to only those matches
    # This is a hack to deal with events like the Crown where there are 3rd place matches
    if any(match.match_number is not None for match in matches):
        matches = [match for match in matches if match.match_number is not None]
