...
export const createMatchesFromSeeds = (
  ...
  // - effectiveSize >= 8 (N >= 9): IBJJF custom. Play-in seeds are pulled in priority
  ...
  //   takes the higher block. Verified against real IBJJF brackets at N=9, 10, 11, 14.
  ...
  const firstRound: Match[] = firstRoundSeeds.map(([redSeed, blueSeed]) => {
    const red = redSeed !== null ? (bySeed.get(redSeed) ?? null) : null;
    const blue = blueSeed !== null ? (bySeed.get(blueSeed) ?? null) : null;
